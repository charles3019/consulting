...
const timeSlots = consultationTimes;

export default function BookConsultation({ today }: { today: string }) {
  ...
  const initialDate = new Date(today + "T12:00:00Z");
  const monthDate = new Date(Date.UTC(initialDate.getUTCFullYear(), initialDate.getUTCMonth() + monthOffset, 1));
  const year = monthDate.getUTCFullYear();
  const month = monthDate.getUTCMonth();
  const currentMonthName = new Intl.DateTimeFormat("en-GB", { month: "long", year: "numeric", timeZone: "UTC" }).format(monthDate);
  const daysInMonth = Array.from({ length: new Date(Date.UTC(year, month + 1, 0)).getUTCDate() }, (_, i) => i + 1);
  const startDayPadding = (monthDate.getUTCDay() + 6) % 7;
  const dateValue = selectedDate ? year * 10000 + (month + 1) * 100 + selectedDate : 0;
  const changeMonth = (offset: number) => { setMonthOffset(offset); setSelectedDate(null); setSelectedTime(""); };
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const result = await submitConsultationRequest({ ...formData, type: selectedType, date: dateValue, time: selectedTime });
      if (result.success) setIsBooked(true);
    ...
    <div className="relative w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 space-y-16">
      ...
      <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
        ...
        <p className="text-sm text-slate-400 leading-relaxed">
          Select a service and your preferred weekday and time. We will contact you to confirm availability.
        </p>
      </div>
        ...
        <div className="glass-panel p-8 sm:p-12 rounded-2xl border border-cyan-500/30 max-w-2xl mx-auto text-center space-y-6">
          ...
          <div className="p-4 rounded-xl bg-slate-900 border border-white/5 space-y-3 text-left text-sm max-w-md mx-auto">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white">{formatBookingDate(dateValue)}</span>
            </div>
            <div className="flex justify-between">
          ...
          <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
            We have received your preferred date and time. Our team will contact you to confirm availability and arrange the session.
          </p>
                ...
                <div className="space-y-1.5 text-xs text-slate-400">
                  ...
                  {selectedDate && (
                    <div className="flex items-center space-x-2">
                      ...
                      <span>Date: {formatBookingDate(dateValue)}</span>
                    </div>
                  )}
                  {selectedTime && (
                    <div className="flex items-center space-x-2">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center px-1">
                      ...
                      <div className="flex space-x-1">
                        <button type="button" aria-label="Previous month" disabled={monthOffset === 0} onClick={() => changeMonth(monthOffset - 1)} className="p-1 rounded text-slate-300 disabled:opacity-30">
                          <ChevronLeft className="w-4 h-4" />
                        </button>
                        <button type="button" aria-label="Next month" disabled={monthOffset === 11} onClick={() => changeMonth(monthOffset + 1)} className="p-1 rounded text-slate-300 disabled:opacity-30">
                    ...
                    <div className="grid grid-cols-7 gap-1">
                      ...
                      {daysInMonth.map((day) => {
                        ...
                        const isUnavailable = isWeekend || year * 10000 + (month + 1) * 100 + day <= bookingDateNumber(today);
              ...
              <div className="hidden" aria-hidden="true"><label htmlFor="booking-website">Leave this field empty</label><input id="booking-website" name="website" tabIndex={-1} autoComplete="off" value={formData.website} onChange={e => setFormData({ ...formData, website: e.target.value })} /></div>
                  ...
                  <div className="pt-4 border-t border-white/5 flex justify-end">
                    ...
                    >
                      ...
                      <span>{pending ? "Sending..." : "Request Consultation"}</span>
                    </button>
                  </div>
                </form>
